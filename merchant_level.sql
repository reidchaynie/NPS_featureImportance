...
select 
  order_id 
  ,listagg(distinct LINE_ITEM_DERIVED_DETAILS.MERCHANT_ID,',') as MERCHANT_ID_
 ,listagg(distinct LINE_ITEM_DERIVED_DETAILS.MERCHANT_name,',') as MERCHANT_name_
...
where 
  ...
  and LINE_ITEM_DERIVED_DETAILS.merchant_id in 
    (select id from PROD_MART_GOLDBELLY_DB.ANALYTICS_MART.merchants where is_featured and is_Available) -- narrows to currently active products  
  and not IS_SUBSCRIPTION_OG and not IS_SUBSCRIPTION_FULFILLMENT
  and not IS_CORP_ORDER
and not IS_CANCELED 
group by 1,4
having 
  merchant_id_ not like '%,%' --removes orders containing multiple products
),
...
select 
  1 as join_on_me 
  ,retention_counts.MERCHANT_name_
  ,retention_counts.merchant_id_
  ,total_oad 
  ,total_Retained  
...
from 
  retention_counts 
left join volume_Factor_calc_2022 on retention_counts.MERCHANT_name_ = volume_Factor_calc_2022.MERCHANT_name_
)
,
